...
| **High** | **Frontend: Add `updatePassiveSuggestion` action** | - Create and export a new `updatePassiveSuggestion` action.<br>- This action will find a passive suggestion by its ID and update its `suggestions` array with the AI-generated rewrite.                                                 | `src/store/suggestion/suggestion.actions.ts`                                                                                                                                                                                                                                                                                             | ☐      |
| **High** | **Frontend: Connect action to `usePassiveRewrite` hook** | - Import and call the `updatePassiveSuggestion` action from within the `usePassiveRewrite` hook to close the loop on the AI suggestion flow.                                                                                        | `src/hooks/usePassiveRewrite.ts`                                                                                                                                                                                                                                                                                                           | ☐      |
